...
class VariantHandler(ABC):
  ...
  async def playlist(self, request: web.Request) -> web.Response:
    ...
    cache_headers = {'Access-Control-Allow-Origin': '*', 'Cache-Control': 'no-store'}

    if msn is None and part is None:
      future = self.m3u8.plain()
      if future is None:
        return web.Response(headers=cache_headers, status=400, content_type="application/x-mpegURL")

      result = await future
      return web.Response(headers=cache_headers, text=result, content_type="application/x-mpegURL")
    else:
      if msn is None:
        return web.Response(headers=cache_headers, status=400, content_type="application/x-mpegURL")
      msn = int(msn)
