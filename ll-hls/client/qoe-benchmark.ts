 ...
 * 
 * Simulates multiple concurrent clients watching the same HLS stream
 * and collects QoE metrics: Latency, Time to First Frame (TTFF), and Stalls.
 * 
 * Usage:
 *   npx tsx qoe-benchmark.ts --clients 10 --duration 60 --stream http://localhost:8080/live.m3u8
 * 
 * Output:
 *   - CSV file with per-second metrics for each client
...
class QoEBenchmark {
  ...
  async run(): Promise<void> {
    ...
    // Optional warmup pass to avoid cold TTFF; discard metrics
