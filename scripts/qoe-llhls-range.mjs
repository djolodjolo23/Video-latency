...
if (!Number.isFinite(start) || !Number.isFinite(end) || start <= 0 || end < start) {
  console.error("Usage: npm run qoe:llhls:range -- <start> <end> [--duration <sec>]");
  process.exit(1);
}
...
async function runOnce(clients) {
  return new Promise((resolve) => {
    const child = spawn(
      "node",
      ["scripts/qoe-llhls.mjs", String(clients), "--duration", String(durationSec)],
      { stdio: "inherit" }
    );
    currentChild = child;
    let timeout = null;
...
try {
  for (let clients = start; clients <= end; clients += 1) {
    console.log(`\n=== Running LL-HLS QoE for ${clients} clients ===`);
